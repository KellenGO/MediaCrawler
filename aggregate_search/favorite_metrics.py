"""Bounded, sequential enrichment of public favourite counters.
...
async def enrich_favorites(platform, client, rows, sink, *, cache_dir: Path | None = None):
    cache_dir = cache_dir if cache_dir is not None else writable_path(".cache", "favorite_metrics")
    seen = set()
    requested = False
    ...
    for row in rows:
        ...
        if cached is not None:
            metrics, at = cached["metrics"], cached["at"]
            approximate = cached.get("approximate", [])
        else:
            if requested:
                await asyncio.sleep(REQUEST_INTERVAL)
            remaining = deadline - time.monotonic()
            if remaining <= 0:
                raise asyncio.TimeoutError("收藏指标补全超时")
        ...
        sink([{**row, "_favorite_metrics": metrics, "_metrics_updated_at": at,
               "_metrics_approximate": sorted(approximate), "_metrics_cached": cached is not None,
