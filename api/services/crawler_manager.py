...
class CrawlerManager:
    ...
    async def start(self, config: CrawlerStartRequest) -> bool:
        ...
        async with self._lock:
            ...
            # Build command line arguments and subprocess env
            cmd = self._build_command(config)
            env_override = self._build_env(config)

            # Log start information — command must never contain secrets
            entry = self._create_log_entry(f"Starting crawler: {' '.join(cmd)}", "info")
            await self._push_log(entry)

            try:
                # Start subprocess — cookies go via env, NOT argv
    ...
    def _build_command(self, config: CrawlerStartRequest) -> list:
        """Build main.py command line arguments (no secrets)."""
        ...
        # Cookies are never in argv. They go via env only.
        return cmd

    @staticmethod
    def _build_env(config: CrawlerStartRequest) -> dict:
        """Build subprocess environment. Cookies passed via env, NOT argv."""
        env = {"PYTHONUNBUFFERED": "1"}
        if config.cookies:
            env["MEDIACRAWLER_COOKIES"] = config.cookies
        return env
