...
- 进入"账号设置"时自动检测账号状态，并自动同步尚未确认登录的平台。
...
先在浏览器里登录平台、再回到本网站，是账号同步最常见的顺序。进入"账号设置"页时，页面会先读取一次账号状态（GET `/api/search/accounts`），只要检测到**尚未确认已验证登录**的平台，且浏览器扩展已连接、本地 API 可用、当前没有同步或搜索在跑，就自动为这些平台执行一次同步 —— 不必再手动点击"一键同步四个平台"。四个平台都已验证登录时不会发起任何额外请求。

自动同步是安静的：不弹 toast，只在页面上显示一行结果；每个页面生命周期最多自动执行一次；只同步未确认登录的平台（不重新同步另外几个），进度分母随实际平台数变化。它复用与手动同步相同的队列（固定顺序、最大并发 2、全局阻断即停止），因此手动与自动不会叠加执行。

该批回归测试见 `webui/tests/accountBulkSync.test.ts`（`platformsNeedingSync` / `decideAutoSync` 纯函数）和 `tests/test_webui_ui_contract.py`（前端接线契约）。
