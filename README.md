...
- 实时查看爬虫运行状态和日志
- 数据预览和导出
- 多平台聚合搜索：一次输入、多平台同时搜索，结果与状态按平台分组展示
- 搜索浮层：聚焦输入框弹出最近搜索与推荐搜索，支持快捷键与点击外部关闭
- 账号登录状态管理：顶部登录徽章实时显示已验证平台数（N/M），登录失效自动降级并提示一键重新同步
- 账号批量同步：本地已导入的浏览器会话可批量同步为账号条目，无需登录的平台自动降级为公开搜索
- 支持浅色/深色主题切换与简体中文/English 界面
