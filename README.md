...
- **结果内筛选**：按时间、内容类型和关键词（空格分隔）筛选，命中词高亮。
- **收藏与备注**：本地收藏、写备注，支持备份与导入。
...
- 筛选只处理已经返回的结果，不会额外去平台搜索；时间范围按发布时间计算。
- 收藏存在浏览器本地存储（上限 500 条，备注最多 1000 字）。换浏览器或清除网站数据前，先在「本地收藏」点**「备份全部收藏」**导出 JSON，之后可导入恢复。
- 导出默认作用于当前筛选下的全部结果，勾选后只导出选中项；CSV 带 UTF-8 BOM，Excel 打开中文不乱码。
- 跨平台收藏是**只读**的：不会在平台侧添加、删除或移动你的收藏，每次每平台取最近 20 条。

### 稳定性与账号

- 平台明确限流后进入 60 秒冷却，重复触发依次延长到 120 / 240 / 300 秒；其他平台不受影响。
- 搜索前先做一次本地登录预检，确定搜不了的平台立刻提示「需要登录」，不用白等浏览器启动。
- 打开程序会自动同步尚未登录的平台；账号设置里可查看各平台状态与诊断信息。
...
- **普通用户**：下载 Windows 可执行包，按[使用说明](docs/使用说明.md)操作。
- **开发者**：从源码运行，见下面「从源码运行」。

两种方式使用同一个地址：`http://127.0.0.1:8080`。

> **开始搜索前必须做两件事**：在浏览器登录小红书、抖音、B 站、知乎，然后加载随包的浏览器插件  
...
MediaCrawler.bat                     # 一键启动（需已存在 webui/dist）
```
...
- **8080 端口被占用**：关掉已经运行的本项目实例；程序不会强制抢端口，也不会自动换端口。
...
```text
Web UI / 浏览器插件 → FastAPI → 任务管理 → 隔离 worker → 平台适配层 → 统一结果 → 补全 / 去重 / 排序 / 缓存
```
