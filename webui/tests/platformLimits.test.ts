...
test("默认四个平台都是 20", () => {
  assert.deepEqual(DEFAULT_PLATFORM_LIMITS, {
    xhs: 20, douyin: 20, bilibili: 20, zhihu: 20,
  });
});

test("合法的 1 和 40 被接受", () => {
  assert.equal(normalizePlatformLimit(1), 1);
  assert.equal(normalizePlatformLimit(40), 40);
  assert.equal(parsePlatformLimitInput("1"), 1);
  assert.equal(parsePlatformLimitInput("40"), 40);
});

test("0、41、负数按前端规则校正（夹紧）", () => {
  assert.equal(normalizePlatformLimit(0), 1);
  assert.equal(normalizePlatformLimit(41), 40);
  assert.equal(normalizePlatformLimit(-5), 1);
  assert.equal(parsePlatformLimitInput("0"), 1);
  assert.equal(parsePlatformLimitInput("41"), 40);
...
test("一个字段非法时，其他合法字段保留", () => {
  ...
  assert.equal(parsed.douyin, 20); // 非法字段回退默认
  assert.equal(parsed.zhihu, 20);
});

test("缺失字段回退 20；未知平台被过滤", () => {
  const parsed = parsePlatformLimits({ xhs: 3, myspace: 7, bogus: 9 });
  assert.equal(parsed.xhs, 3);
  assert.equal(parsed.douyin, 20);
  assert.equal(parsed.bilibili, 20);
  assert.equal(parsed.zhihu, 20);
...
test("readPlatformLimits 按字段恢复：部分平台有效时其余用默认", () => {
  ...
  assert.equal(limits.bilibili, 20);
  assert.equal(limits.zhihu, 20);
});
...
test("修改一个平台不改变其他平台", () => {
  ...
  assert.equal(next.douyin, 20);
  assert.equal(next.bilibili, 20);
  assert.equal(next.zhihu, 20);
  assert.equal(base.xhs, 20, "输入对象不被 mutate");
});
...
test("updatePlatformLimit 非法值返回原对象（不改变）", () => {
  ...
  // 越界值按前端规则夹紧后生效（0→1、41→40）
  const clampedLow = updatePlatformLimit(base, "xhs", 0);
  assert.equal(clampedLow.xhs, 1);
  const clampedHigh = updatePlatformLimit(base, "xhs", 41);
  assert.equal(clampedHigh.xhs, 40);
});

test("恢复默认得到四个 20", () => {
  const next = resetPlatformLimits();
  assert.deepEqual(next, { xhs: 20, douyin: 20, bilibili: 20, zhihu: 20 });
});
...
test("selectedPlatformLimits 不 mutate 输入对象", () => {
  const limits: PlatformLimitMap = { xhs: 5, douyin: 20, bilibili: 20, zhihu: 20 };
...
test("MIN/MAX 常量为 1 和 40", () => {
  assert.equal(MIN_PLATFORM_LIMIT, 1);
  assert.equal(MAX_PLATFORM_LIMIT, 40);
});
...
test("完整写入后读取保持一致", () => {
  ...
  assert.deepEqual(readPlatformLimits(storage), { xhs: 4, douyin: 20, bilibili: 20, zhihu: 20 });
});
