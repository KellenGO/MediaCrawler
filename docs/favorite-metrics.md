...
收藏列表先流式返回，再在原浏览器会话内串行读取详情。按内容 ID 合并更新，保留多个收藏夹的归属，不重复增加卡片。仅影响收藏路径。
...
- 详情调用间隔 2 秒，单次最多等待 12 秒，详情阶段总预算 120 秒。整个收藏 worker 最多 180 秒，主管理器最多等待 190 秒。这些值不是平台保证的安全额度。
- 详情错误立即结束该平台的补全；列表和已完成的详情结果保留，任务可以返回 partial。未完成条目标记 failed。
- 成功响应中的计数缓存 6 小时，位置 `.cache/favorite_metrics/`。缓存仅含计数、近似字段和采集时间；文件名为平台、类型及内容 ID 的摘要。不写入令牌、Cookie、正文或收藏夹名称。
...
- `aggregate_search/favorite_metrics.py`：字段提取、串行补全、缓存与计数质量标记。
