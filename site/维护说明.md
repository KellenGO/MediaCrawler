...
| `guide.html` | 使用说明：安装启动、搜索、排序筛选、收藏导出、账号同步、常见问题 | 已经下载、想知道怎么用的人 |
...
`MediaCrawler-Windows-x64.zip` 解压后是 `MediaCrawler/` 目录，里面：

| 文件 | 作用 | 用户要不要点 |
| --- | --- | --- |
| `四野.exe` | 托盘启动器（无窗口）：隐藏启动后端、自动打开网页、右下角托盘菜单 | **要**，推荐入口 |
| `MediaCrawler.exe` | 后端服务 + 平台 worker（带控制台） | 不必；想直接看日志时可以双击 |
...
它把 `MediaCrawler.exe --no-browser` 以隐藏窗口方式拉起来，输出重定向到
...
落地页主 CTA 用的是稳定直链 `releases/latest/download/MediaCrawler-Windows-x64.zip`，它要生效必须同时满足：

1. **asset 文件名必须一字不差**：`MediaCrawler-Windows-x64.zip`（建议把 `.sha256` 一起传，方便用户校验）。
   改名、加版本号后缀（如 `-v0.1.0`）都会让直链失效。
