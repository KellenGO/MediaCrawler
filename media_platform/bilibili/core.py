        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while remaining > 0 and (page - start_page + 1) * bili_limit_count <= config.CRAWLER_MAX_NOTES_COUNT + bili_limit_count:
                ...
                if not self._fetch_details():
                    # ── 聚合搜索轻量列表模式 ─────────────────────────────
                    # 搜索接口 /search/type 的扁平列表项已包含展示所需 MVP
                    # 字段：直接把列表项交给 result_sink，精确裁剪 remaining，
                    # 不调用 get_video_info_task、不写数据库、不抓评论/媒体。
                ...
                task_list = []
                try:
                    task_list = [self.get_video_info_task(aid=v.get("aid"), bvid="", semaphore=semaphore) for v in videos_to_fetch]
                ...
                valid_videos = [v for v in video_items if v]
                remaining -= len(valid_videos)
                # ── aggregate-search hook: push native results to sink ──
                self._result_sink_call(valid_videos)
                for video_item in valid_videos:
